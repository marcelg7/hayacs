...
<div x-show="activeTab === 'events'" x-cloak class="bg-white dark:bg-{{ $colors['card'] }} shadow rounded-lg overflow-hidden">
    <div class="px-4 py-5 sm:px-6 border-b border-gray-200 dark:border-{{ $colors['border'] }}">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
            ...
            @endphp
            @if($rebootCount24h >= 3)
                <span class="inline-flex items-center self-start px-3 py-1 rounded-full text-sm font-medium bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200">
            ...
            @elseif($rebootCount7d >= 5)
                <span class="inline-flex items-center self-start px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
                    <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                    </svg>
    ...
    <div class="sm:hidden divide-y divide-gray-200 dark:divide-{{ $colors['border'] }}">
                ...
                <div class="flex items-start justify-between gap-2">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span class="text-xs text-gray-400 dark:text-{{ $colors['text-muted'] }}">
                        ...
                        @if($event->command_key || $event->details)
                            <div class="mt-2 text-xs">
                                @if($event->command_key)
                                    <span class="font-mono bg-gray-100 dark:bg-{{ $colors['bg'] }} px-1 rounded break-all">{{ Str::limit($event->command_key, 40) }}</span>
                                @endif
                                @if($event->details)
                                    <div class="mt-1 text-gray-500 dark:text-{{ $colors['text-muted'] }}">
                                        @foreach($event->details as $key => $value)
                                            <div><span class="text-gray-400">{{ $key }}:</span> {{ Str::limit($value, 50) }}</div>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
        ...
        @empty
            <div class="p-8 text-center text-sm text-gray-500 dark:text-{{ $colors['text-muted'] }}">
