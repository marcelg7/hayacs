    ...
    <div class="bg-white dark:bg-{{ $colors['card'] }} shadow overflow-hidden sm:rounded-lg mb-6" x-data="{
        ...
        async applyConfig() {
            ...
            try {
                ...
                // Include guest password if provided (otherwise backend will auto-generate)
                if (this.enableGuest && this.guestPassword) {
                    payload.guest_password = this.guestPassword;
                }
                const response = await fetch(`/api/devices/${deviceId}/standard-wifi`, {
                    method: 'POST',
                    headers: {
                ...
                if (result.tasks && result.tasks.length > 0) {
                    ...
                    // Show auto-generated guest password (only if user didn't provide one)
                    if (result.guest_password && !this.guestPassword) {
                        setTimeout(() => {
