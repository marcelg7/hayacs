{{-- WiFi SSID Card Partial (supports both TR-098 and TR-181) --}}
@php
    // Normalize field names between TR-098 and TR-181
    $ssidName = $config['SSID'] ?? '';
    $enabled = ($config['Enable'] ?? '0') === '1' || ($config['Enable'] ?? '') === 'true';
    $channel = $config['Channel'] ?? '';
    $autoChannel = ($config['AutoChannelEnable'] ?? '0') === '1' || ($config['AutoChannelEnable'] ?? '') === 'true';
    $ssidBroadcast = ($config['SSIDAdvertisementEnabled'] ?? '1') === '1' || ($config['SSIDAdvertisementEnabled'] ?? '') === 'true';
    $status = $config['Status'] ?? 'Unknown';
    $standard = $config['Standard'] ?? $config['OperatingStandards'] ?? '-';
    $bandwidth = $config['OperatingChannelBandwidth'] ?? $config['X_000631_OperatingChannelBandwidth'] ?? '';
    $autoBandwidth = empty($bandwidth) || $bandwidth === 'Auto';

    // Check if this is a Calix device (can reveal passwords)
    $isCalix = $isCalix ?? false;

    // Password field - TR-181 uses KeyPassphrase, TR-098 uses X_000631_KeyPassphrase
    // Calix devices (TR-098) expose passwords in clear text via X_000631_KeyPassphrase
    $password = '';
    if ($isDevice2) {
    ...
    // Security mode for display
    $securityMode = $config['SecurityMode'] ?? '';
    if (empty($securityMode) && !empty($config['BeaconType'])) {
        $securityMode = $config['BeaconType'];
    }
...
<div class="border {{ $cardBorder }} rounded-lg p-4 {{ $cardBg }} flex flex-col transition-colors">
    <form @submit.prevent="async (e) => {
        ...
        <input type="hidden" name="data_model" value="{{ $isDevice2 ? 'TR-181' : 'TR-098' }}">
        ...
        <div class="space-y-2 flex-1">
            ...
            <!-- Password -->
            <div x-data="{ showPassword: false, currentPassword: '{{ $isCalix && $password ? addslashes($password) : '' }}' }">
                <label class="block text-xs font-medium text-gray-600 dark:text-{{ $colors['text-muted'] }}">Password</label>
                <div class="relative mt-1">
                    <input :type="showPassword ? 'text' : 'password'" name="password" value=""
                           minlength="8" maxlength="63"
                           :placeholder="showPassword && currentPassword ? currentPassword : 'Leave blank to keep current'"
                           class="block w-full text-sm rounded-md border-gray-300 dark:border-{{ $colors['border'] }} dark:bg-{{ $colors['card'] }} dark:text-{{ $colors['text'] }} shadow-sm focus:border-blue-500 focus:ring-blue-500 pr-10">
                    @if($isCalix && $password)
                ...
                @if($isCalix && $password)
                <p class="mt-0.5 text-xs text-green-600 dark:text-green-400">Current password available - click eye to reveal</p>
                @endif
            </div>
