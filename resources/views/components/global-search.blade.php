...
<div x-data="globalSearch()" x-init="init()" @keydown.window="handleGlobalKeydown($event)" class="relative">
    ...
    <div class="relative">
        ...
        <input x-ref="searchInput"
               ...
               type="text"
               placeholder="Search..."
               class="w-56 lg:w-72 xl:w-80 pl-10 pr-10 py-2 text-sm bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-{{ $colors['primary'] ?? 'indigo' }}-500 focus:border-transparent text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 transition-all duration-150">
        ...
        <div x-show="!query && !isFocused" class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
            <kbd class="px-1.5 py-0.5 text-xs font-mono text-gray-400 bg-gray-200 dark:bg-gray-600 rounded">/</kbd>
        </div>
    ...
    <div x-show="showResults"
         ...
         @mousedown.prevent
         class="absolute top-full left-0 mt-2 w-[30rem] max-w-[calc(100vw-2rem)] bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden z-50"
         x-cloak>

        <!-- Results Container -->
        <div class="max-h-[70vh] overflow-y-auto" x-ref="resultsContainer">
            <!-- Loading State -->
            <div x-show="loading" class="px-4 py-6 text-center">
                <svg class="animate-spin h-5 w-5 mx-auto text-{{ $colors['primary'] ?? 'indigo' }}-600" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
            </div>

            <!-- Help Text (shown when focused but no query) -->
            <div x-show="!loading && !query && isFocused" class="p-4">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex items-center gap-3 text-gray-600 dark:text-gray-400 whitespace-nowrap">
                        <span class="w-24 shrink-0 text-{{ $colors['primary'] ?? 'indigo' }}-500 font-medium">Device ID</span>
                ...
                <div class="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700 text-xs text-gray-400 flex items-center gap-4">
                    <span class="flex items-center gap-1"><kbd class="px-1.5 py-0.5 bg-gray-200 dark:bg-gray-600 rounded">↑↓</kbd> navigate</span>
                    <span class="flex items-center gap-1"><kbd class="px-1.5 py-0.5 bg-gray-200 dark:bg-gray-600 rounded">↵</kbd> select</span>
                    <span class="flex items-center gap-1"><kbd class="px-1.5 py-0.5 bg-gray-200 dark:bg-gray-600 rounded">esc</kbd> close</span>
                </div>
            </div>

            <!-- No Results -->
            <div x-show="!loading && query && results.length === 0 && searched" class="px-4 py-6 text-center">
            ...
            <!-- Results -->
            <template x-for="(category, catIndex) in results" :key="category.category">
                <div class="border-b border-gray-100 dark:border-gray-700 last:border-0">
                    <!-- Category Header -->
                    <div class="px-3 py-1.5 bg-gray-50 dark:bg-gray-900/50 sticky top-0">
                        <div class="flex items-center gap-2">
                            <template x-if="category.icon === 'device'">
                                <svg class="w-3.5 h-3.5 text-{{ $colors['primary'] ?? 'indigo' }}-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <template x-for="(item, itemIndex) in category.items" :key="item.id + '-' + itemIndex">
                        <a :href="item.url"
                           ...
                           class="flex items-center gap-3 px-3 py-2.5 hover:bg-{{ $colors['primary'] ?? 'indigo' }}-50 dark:hover:bg-{{ $colors['primary'] ?? 'indigo' }}-900/20 cursor-pointer transition-colors"
                           :class="{'bg-{{ $colors['primary'] ?? 'indigo' }}-50 dark:bg-{{ $colors['primary'] ?? 'indigo' }}-900/20': isActive(catIndex, itemIndex)}">
        ...
        <div x-show="results.length > 0 && !loading" class="px-3 py-2 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between text-xs text-gray-400">
                <span x-text="total + ' result' + (total !== 1 ? 's' : '')"></span>
...
function globalSearch() {
    return {
        ...
        init() {
            // Nothing needed here - / key handled by handleGlobalKeydown
        },
        ...
        navigateDown() {
            ...
            const currentCategory = this.results[this.activeCategory];
            if (this.activeItem < currentCategory.items.length - 1) {
                this.activeItem++;
            } else if (this.activeCategory < this.results.length - 1) {
                this.activeCategory++;
                this.activeItem = 0;
            }
            this.scrollToActive();
        ...
        navigateUp() {
            ...
            if (this.activeItem > 0) {
                ...
                this.activeCategory--;
                this.activeItem = this.results[this.activeCategory].items.length - 1;
            }
            this.scrollToActive();
        },
        ...
        selectResult() {
            ...
            const category = this.results[this.activeCategory];
